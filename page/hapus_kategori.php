...
if(isset($_GET['id_kategori'])){
	...
	if(mysqli_num_rows($cek) > 0){
		...
		if($del){
			echo '<script>alert("Berhasil menghapus data."); document.location="?page=kategori";</script>';
		}else{
			echo '<script>alert("Gagal menghapus data."); 
			document.location="?page=kategori";</script>';
		}
	}else{
		echo '<script>alert("ID tidak ditemukan di database."); document.location="?page=kategori";</script>';
	}
}
